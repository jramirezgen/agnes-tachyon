...
if [ "$ENABLE_LIBRARIAN_LEARNER" = "1" ]; then
  if [ -f "$LEARN_SCRIPT" ]; then
    # Defaults tuned for lower RAM/SSD pressure while keeping continuous learning.
    LD_LIBRARY_PATH="$PYTHON_LD_LIBRARY_PATH" MINIVERSE_SERVER_URL="$SERVER_URL" TACHYON_URL="$TACHYON_URL" AGNES_TRAINER_PYTHON="$MINIVERSE_PYTHON" AGNES_HOME="$AGNES_HOME" AGNES_DATA_DIR="$AGNES_DATA_DIR" LIBRARIAN_SWIFT_EXPORT_MERGED="${LIBRARIAN_SWIFT_EXPORT_MERGED:-1}" LIBRARIAN_MERGE_INTERVAL_SEC="${LIBRARIAN_MERGE_INTERVAL_SEC:-21600}" LIBRARIAN_SWIFT_KEEP_RUNS="${LIBRARIAN_SWIFT_KEEP_RUNS:-1}" LIBRARIAN_SWIFT_KEEP_MERGED="${LIBRARIAN_SWIFT_KEEP_MERGED:-1}" LIBRARIAN_SWIFT_EVAL_CASES="${LIBRARIAN_SWIFT_EVAL_CASES:-3}" LIBRARIAN_SWIFT_EVAL_MAX_TOKENS="${LIBRARIAN_SWIFT_EVAL_MAX_TOKENS:-128}" LIBRARIAN_SWIFT_PAUSE_SEC="${LIBRARIAN_SWIFT_PAUSE_SEC:-90}" TORCHINDUCTOR_COMPILE_THREADS="${TORCHINDUCTOR_COMPILE_THREADS:-1}" OMP_NUM_THREADS="${OMP_NUM_THREADS:-2}" MKL_NUM_THREADS="${MKL_NUM_THREADS:-2}" NUMEXPR_NUM_THREADS="${NUMEXPR_NUM_THREADS:-2}" "$MINIVERSE_PYTHON" -u "$LEARN_SCRIPT" > generated/librarian_learn.log 2>&1 &
    LEARN_PID=$!
