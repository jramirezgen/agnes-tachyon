...
ENABLE_LIBRARIAN_LEARNER="${ENABLE_LIBRARIAN_LEARNER:-0}"
if [ "$ENABLE_LIBRARIAN_LEARNER" = "1" ]; then
  if [ -f "$LEARN_SCRIPT" ]; then
    LD_LIBRARY_PATH="$PYTHON_LD_LIBRARY_PATH" MINIVERSE_SERVER_URL="$SERVER_URL" TACHYON_URL="$TACHYON_URL" AGNES_TRAINER_PYTHON="$MINIVERSE_PYTHON" AGNES_HOME="$AGNES_HOME" AGNES_DATA_DIR="$AGNES_DATA_DIR" "$MINIVERSE_PYTHON" -u "$LEARN_SCRIPT" > generated/librarian_learn.log 2>&1 &
    LEARN_PID=$!
